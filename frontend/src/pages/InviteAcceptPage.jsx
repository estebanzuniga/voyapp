    ...
    <div className="flex min-h-dvh items-center justify-center bg-bg px-4">
      <div className="flex w-full max-w-sm flex-col gap-4 rounded-xl border border-border bg-surface p-6 shadow-sm">
        ...
        {!loading && !error && preview && !preview.valid ? (
          <p className="text-muted">{t('inviteAccept.invalidOrExpired')}</p>
        ) : null}
          ...
          <>
            <div className="flex flex-col gap-1">
              <h1 className="font-display text-lg text-ink text-balance">
                {t('inviteAccept.invitedTo', { title: preview.tripTitle })}
              </h1>
              <p className="text-sm text-muted">
                {t('inviteAccept.body', {
                  permission:
                    preview.permission === 'EDITOR'
                      ? t('inviteAccept.permissionEditor')
                      : t('inviteAccept.permissionViewer'),
                })}
              </p>
            </div>
            ...
            {isAuthenticated ? (
              ...
              >
                {accepting ? t('inviteAccept.joining') : t('inviteAccept.acceptInvite')}
              </button>
            ) : (
              <div className="flex flex-col gap-2">
                <p className="text-sm text-muted">{t('inviteAccept.logInPrompt')}</p>
                <Link
